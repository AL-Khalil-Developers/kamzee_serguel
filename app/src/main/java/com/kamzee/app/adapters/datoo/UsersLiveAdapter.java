                ...
                public void onPermissionResult(Permiso.ResultSet resultSet) {
                    if (resultSet.areAllPermissionsGranted()) {
                        ...
                        if (QuickHelp.isInternetAvailable(mActivity)){
                            ...
                            ParseQuery<BlockStreamingModel> blockStreamingModelParseQuery = BlockStreamingModel.getBlockStreamingQuery();
                            blockStreamingModelParseQuery.whereEqualTo(BlockStreamingModel.BLOCKED_USER_ID, ParseUser.getCurrentUser().getObjectId());
                            blockStreamingModelParseQuery.findInBackground((objects, e) -> {
                                ...
                                if (e == null && objects.size() > 0){
                                    ...
                                    for (BlockStreamingModel blockStreamingModel : objects){
                                        if (blockStreamingModel.getOwner().getObjectId().equals(liveStreamModel.getAuthor().getObjectId())){
                                            QuickHelp.showToast(mActivity, mActivity.getString(R.string.you_are_banned), true);
                                        }
                                    }
