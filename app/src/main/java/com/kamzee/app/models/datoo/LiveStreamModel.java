...
public class LiveStreamModel extends ParseObject {
    ...
    public User getAuthor() {
        //return (User) getParseObject(AUTHOR).fetchIfNeeded();
    ...
    public void addBlocker(User blocker) {
        addUnique(BLOCKERS, blocker);
    }

    public void removeBlocker(User blocker) {

        ArrayList<User> users = new ArrayList<>();
        users.add(blocker);

        removeAll(BLOCKERS, users);
    }

    /*public List<String> getBlockersIds(){
        List<String> a = new ArrayList<>();

        if (getList(BLOCKERS) != null && getList(BLOCKERS).size() > 0){
            return getList(BLOCKERS);
        } else {
            return a;
        }
    }*/
