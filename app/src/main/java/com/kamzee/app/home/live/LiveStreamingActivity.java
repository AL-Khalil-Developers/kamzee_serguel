...
import com.kamzee.app.adapters.datoo.GiftLiveAdapter;
import com.kamzee.app.adapters.datoo.GiftLiveFooterAdapter;
import com.kamzee.app.adapters.datoo.LiveMessageAdapter;
...
import com.kamzee.app.models.datoo.ReportModel;
import com.kamzee.app.models.datoo.User;
import com.kamzee.app.modules.circularimageview.CircleImageView;
import com.kamzee.app.utils.StatusBarUtil;
import com.kamzee.app.utils.liveUtils.VideoGridContainer;
...
import static com.kamzee.app.app.Constants.VIDEO_DIMENSIONS;
import static com.kamzee.app.models.datoo.GiftModel.SEND_GIFT_PARAM;
...
public class LiveStreamingActivity extends AppCompatActivity {
    ...
    public TextView mTimeText, mViewersText, mTokensText, mStreamerName;
    ...
    CircleImageView mAvatar, mStreamerAvatar;
    ...
    ImageView mFullScreenGiftImage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFullScreenGiftImage = findViewById(R.id.liveStreaming_fullscreenGiftImage);
        mFullScreenGiftImage.setVisibility(View.GONE);
        ...
        mStreamerName = findViewById(R.id.viewstreamingHeader_name);
        mStreamerAvatar = findViewById(R.id.viewstreamingHeader_streamerImage);
        ...
        mLiveMessageAdapter = new LiveMessageAdapter(this, mLiveMessages,mCurrentUser);
    ...
    public void initStreamerInfo() {
        ...
        streamerName.setText(mLiveStreamObject.getAuthor().getColFirstName());
    ...
    public void init() {
        ...
        if (mStreamType == LIVE_STREAMING_STREAMER){
            ...
            QuickHelp.getAvatars(mCurrentUser, mStreamerAvatar);
            mStreamerName.setText(mCurrentUser.getColFirstName());
    ...
    private void setupLiveQuery(String objectId) {
        ...
        liveMessageQueryStreamSubscription.handleEvent(SubscriptionHandling.Event.CREATE, (query, liveMessage) -> {
            ...
            if (!liveMessage.getSenderAuthorId().equals(mCurrentUser.getObjectId())){
                ...
                if (liveMessage.getMessageType().equals(LiveMessageModel.MESSAGE_TYPE_GIFT)){
                    runOnUiThread(() -> showFullScreenGift(liveMessage.getLiveGift()));
                }
            }

            if (liveMessage.getBannedUser() != null && liveMessage.getBannedUser().equals(mCurrentUser)){

                finishStreaming(false);

                QuickHelp.showToast(LiveStreamingActivity.this, getString(R.string.you_have_been_banned), true);
            }
        ...
        if (isFollowingUser){
            ...
            follow.setOnClickListener(v -> {
                ...
                if (mMoreBottomSheet != null && mMoreBottomSheet.isShowing()){
                    ...
                    followModelParseQuery.include(FollowModel.TO_AUTHOR);
                    followModelParseQuery.include(FollowModel.FROM_AUTHOR);
    ...
    public void sendLiveGift(GiftModel giftModel){
        ...
        if (mCurrentUser.getCoins() >= giftModel.getCoins()){

            mCurrentUser.removeCoin(giftModel.getCoins());
            mCurrentUser.saveInBackground();
            ...
            ParseCloud.callFunctionInBackground(SEND_GIFT_PARAM, params, (FunctionCallback<String>) (result, e) -> {
                ...
                if (e == null){
                    ...
                    mLiveStreamObject.setStreamingTokens(diamonds);
                    mLiveStreamObject.saveInBackground();

                    runOnUiThread(() -> showFullScreenGift(giftModel));
    ...
    public void showFullScreenGift(GiftModel giftModel){
        ...
        if (giftModel.getGiftFile().getUrl().endsWith(".gif")){

            mFullScreenGiftImage.setVisibility(View.VISIBLE);
            QuickHelp.showGifFileFullScreen(this, mFullScreenGiftImage, giftModel.getGiftFile().getUrl());

        } else if (giftModel.getGiftFile().getUrl().endsWith(".json")){
        ...
        if (giftModel.getGiftFileSound() != null && !giftModel.getGiftFileSound().getUrl().isEmpty()){
            try {
                QuickHelp.playAudio(this, giftModel.getGiftFileSound().getUrl());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void banUser(User user) {
        ...
        liveMessage.setSenderAuthorId(mCurrentUser.getObjectId());
        //liveMessage.setMessage(text);
        liveMessage.setBannedUser(user);
        liveMessage.setMessageType(LiveMessageModel.MESSAGE_TYPE_BAN);
